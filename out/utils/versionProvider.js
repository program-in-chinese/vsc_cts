...
class TypeScriptVersion {
    ...
    get version() {
        ...
        const tsdkVersion = vscode_1.workspace.getConfiguration().get('ctsscript.tsdk_version', undefined);
        if (tsdkVersion) {
...
class TypeScriptVersionProvider {
    ...
    get bundledVersion() {
        try {
            const bundledVersion = new TypeScriptVersion(path.dirname(require.resolve('ctsscript/lib/ctsserver.js')), '');
        ...
        vscode_1.window.showErrorMessage(localize('noBundledServerFound', 'VS Code\'s ctsserver was deleted by another application such as a misbehaving virus detection tool. Please reinstall VS Code.'));
