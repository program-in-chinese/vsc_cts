...
export enum 单词属性 {
    ...
    是空白 = 1 << 20,
    不需要翻译 = 全为数字 | 全为符号或数字,
    需要翻译时确定 = 是复数 | 是反义词 | 是简写,
    需要前缀 = 全为大写 | React标签 | 是接口,
    需要后缀 = 全为大写 | 是简写 | 第一个字母为大写,
...
export function 插入标识符别名数据缓存(标识符组: 标识符别名 | 标识符别名[], 是否写入库?: boolean) {
    ...
    for (let 标识符 of 标识符组) {
        ...
        if (!原内容) {
            ...
            标识符库[标识符.文本] = ({ ...原内容, ...标识符别名入库转换(标识符) } as 翻译别名)
        }
    }
    if (是否写入库) {
        写入词典库文件()
...
export function 取标识符别名库储存(标识符: 标识符别名 | string) {
    ...
    let 值 = 读取别名库文件().标识符别名库[索引 as string]
    return 标识符别名出库转换(值)
}
...
function 创建词典库目录及文件() {
    let 路径 = 结合路径(系统.取系统临时文件目录(), "CHTypescript")
    let 文件名 = "CTS_词典自动翻译储存库.json"
    if (!系统.目录存在(路径)) {
        系统.创建目录(路径)
    }
    if (!系统.文件存在(文件名)) {
        系统.写文件(结合路径(路径, 文件名), `{"标识符别名库":{}}`)
    }
}

function 结合路径(path1: string, path2: string) {
...
function 需要翻译时确定(单词: 标识符别名) {
    return (单词.属性 & 单词属性.需要翻译时确定) !== 0
}
function 需要前缀(单词: 标识符别名) {
    return (单词.属性 & 单词属性.需要前缀) !== 0
}
function 需要后缀(单词: 标识符别名) {
    return (单词.属性 & 单词属性.需要后缀) !== 0
}

function 需要范围检查(单词: 标识符别名) {
    return (单词.属性 & 单词属性.需要范围检查) !== 0
}
function 需要语言服务支持(单词: 标识符别名) {
